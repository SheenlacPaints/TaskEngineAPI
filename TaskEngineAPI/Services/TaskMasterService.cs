    ...
    public class TaskMasterService : ITaskMasterService
    {
        ...
        WHERE ID = @processid";
                ...
                if (isOutboundIntegration)
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("SELECT cinoutboundapi_id FROM tbl_process_engine_master WHERE id = @cprocessid", conn))
                    {
                        ...
                        using (SqlDataReader sdr = await cmd.ExecuteReaderAsync())
                        {
                            while (await sdr.ReadAsync())
                            {
                                ...
                                GetTaskList p = new GetTaskList
                                {
                                    ...
                                    createdbyavatar = sdr.IsDBNull(sdr.GetOrdinal("ccreatedbyavatar")) ? string.Empty : Convert.ToString(sdr["ccreatedbyavatar"]),
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        ...
                        using (SqlDataReader reader = await cmd.ExecuteReaderAsync())
                        {
                            while (await reader.ReadAsync())
                            {
                                ...
                                await InoutboundAPIdata(conn, mapping, itaskno, cTenantID, ID);
                                result.Add(mapping);
                            }
                        }
                    }
        ...
        private async Task InoutboundAPIdata(SqlConnection conn, GettaskinboxbyidDTO mapping, int itaskno, int tenantID, int ID)
        {
            string sql = @"SELECT 
        (SELECT id, capi_method, capi_url, cbody, capi_params, capi_headers 
         FROM tbl_users_api_sync_config 
                        ...
                        using (SqlDataReader reader = await cmd.ExecuteReaderAsync())
                        {
                            while (await reader.ReadAsync())
                            {
                                ...
                                var mapping = new GettaskHolddatabyidDTO
                                {
                                    ...
                                    cmeta_response = reader["cmeta_response"]?.ToString() ?? "",
                                    nis_external_api_enabled= reader.SafeGetBoolean("showTimeline"),
                                    nexternal_api_id = reader.SafeGetInt("nexternal_api_id"),// Convert.ToInt32(reader["nexternal_api_id"]),
                                    timeline = new List<TimelineDTO>(),
                                    board = new List<GetprocessEngineConditionDTO>(),
                                    meta = new List<processEnginetaskMeta>(),
                                    approvers = new List<PreviousapproverDTO>(),
     ...
     where a.citaskno=@TaskNo and a.ctenant_id=@TenantID";
                ...
                using (SqlConnection conn = new SqlConnection(connStr))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        ...
                        using (SqlDataReader reader = await cmd.ExecuteReaderAsync())
                        {
                            while (await reader.ReadAsync())
                            {
                                // assign API response once
                                if (string.IsNullOrEmpty(result.capi_response))
                                {
                                    result.capi_response = reader["cmeta_response"]?.ToString() ?? "";
                                }
                    ...
                    using (SqlDataReader sdr = await cmd.ExecuteReaderAsync())
                    {
                        while (await sdr.ReadAsync())
                        {
                            GetTaskList p = new GetTaskList
                            {
                                ...
                                createdbyavatar = sdr["createdbyavatar"]?.ToString() ?? "",
        ...
        public async Task<bool> holdwhatappnotificationAsync(updatetaskDTO model, int cTenantID, string username)
        {
            var connStr = _config.GetConnectionString("Database");
            ...
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                ...
                {
                    string checkQuery = @"
                SELECT a.itaskno, b.cprocess_id,b.ctask_name,b.ccreated_by
                FROM tbl_taskflow_detail a
                INNER JOIN tbl_taskflow_master b ON a.iheader_id = b.ID 
                WHERE a.ID = @ID";
                 ...
                 WHERE cuserid = @sender and nIs_deleted=0";
                    ...
                    using (SqlCommand checkCmd = new SqlCommand(checkQuery, conn, transaction))
                    {
                        ...
                        using (var reader = await checkCmd.ExecuteReaderAsync())
                        {
                            if (await reader.ReadAsync())
                            {
                                ...
                                processId = reader["cprocess_id"] as int?;
                                ctask_name = reader["ctask_name"]?.ToString() ?? "";
                                initiator = reader["ccreated_by"]?.ToString() ?? "";
                            }
                            else
                            {
                                reader.Close();
                    ...
                    using (SqlCommand recipientCmd = new SqlCommand(getRecipientQuery, conn, transaction))
                    {
                        recipientCmd.Parameters.AddWithValue("@initiator", initiator);
                        using (var reader = await recipientCmd.ExecuteReaderAsync())
                        {
                            if (await reader.ReadAsync())
                            {
        ...
        public async Task<bool> RejectwhatappnotificationAsync(updatetaskDTO model, int cTenantID, string username)
        {
            var connStr = _config.GetConnectionString("Database");
        ...
        public async Task<string> Getdepartmentdashboard(int cTenantID, string username, string? searchText = null)
        {
            ...
            {
                using (var con = new SqlConnection(_config.GetConnectionString("Database")))
                using (var cmd = new SqlCommand("sp_get_progovex_department_dashboard", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@tenentid", cTenantID);
                    cmd.Parameters.AddWithValue("@userid", username);
        ...
        public async Task<string> GetNotificationsdashboard(int cTenantID, string username, string? searchText = null)
        {
            ...
            {
                using (var con = new SqlConnection(_config.GetConnectionString("Database")))
                using (var cmd = new SqlCommand("sp_get_progovex_Notifications_dashboard", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@tenentid", cTenantID);
                    cmd.Parameters.AddWithValue("@userid", username);
